    ...
    <main className="uk-flex-auto uk-section">
        ...
        <header>
          <ul className="uk-flex">
            {data[0].attributes.categories.data.map(cat => (
              <li key={cat.id}>
                <a href={`/category/${cat.attributes.slug}`}>{cat.attributes.name}</a>
              </li>
            ))}
          </ul>
          {/* <figure>
        ...
        <footer>
          <p className="uk-article-meta">
            ...
            <time dateTime={data[0].attributes.createdAt}>
              {new Date(data[0].attributes.createdAt).toLocaleDateString('en-us', { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' })}
            </time>
          </p>
          <p className="uk-article-meta">
            Published on{' '}
            <time dateTime={data[0].attributes.publishedAt}>
              {new Date(data[0].attributes.publishedAt).toLocaleDateString('en-us', { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' })}
            </time>
          </p>
          <p className="uk-article-meta">
            Updated on{' '}
            <time dateTime={data[0].attributes.updatedAt}>
              {new Date(data[0].attributes.updatedAt).toLocaleDateString('en-us', { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' })}
            </time>
          </p>
        </footer>
      </article>
      <section className="uk-container uk-container-xsmall uk-margin-top">
        ...
        {data[0].attributes.reviews.data.map(review => (
          <article key={review.id} className="uk-comment uk-comment-secondary uk-margin">
            <header className="uk-comment-header">
              <div className="uk-grid-medium uk-flex-middle" data-uk-grid>
                <div className="uk-width-auto">
                  <span className="uk-margin-small-right uk-icon" data-uk-icon="icon: user; ratio: 2"></span>
                </div>
                <div className="uk-width-expand">
                  <h4 className="uk-comment-title uk-margin-remove">
                    <span className="uk-link-reset">{review.attributes.author}</span>
                  </h4>
                    ...
                    <li>
                      <small>
                        ...
                        <time dateTime={review.attributes.publishedAt}>
                          {new Date(review.attributes.publishedAt).toLocaleDateString('en-us', { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' })}
                        </time>
                      </small>
                    </li>
                  </ul>
